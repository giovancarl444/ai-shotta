...
export default class GitHubModelsClient {
  constructor({ model } = {}) {
    if (process.env.VIA_API !== "true") {
      throw new Error(
        "🚫 Direct instantiation disallowed—use POST /api/chat instead."
      );
    }
  ...
  async chat(messages, opts = {}) {
    ...
    const payload = {
      ...
      user:        opts.user,
      // do *not* set publisher here; let our retry do it
    };
