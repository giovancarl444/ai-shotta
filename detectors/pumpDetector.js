...
async function handleNewToken(address) {
  ...
  try {
    ...
    const entry = enriched
      ...
      : { address, detectedAt: new Date().toISOString(), status: states.RETRY_LATER, source: 'pump.fun' };
    queue.push(entry);
    writeQueue(queue);
    logEvent('info', `Queued ${enriched?.symbol || address} as ${entry.status}`);
  } catch (err) {
...
function startWebSocket() {
  ...
  ws.on('open', () => {
    logEvent('info', 'Pump.fun WS connected');
